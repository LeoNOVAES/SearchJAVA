                    ...
                    if(nome.length > 0 && email.length > 0 && senha.length > 0 && senhaConf.length > 0){
                        ...
                        if(senhaConf != senha){
                         ...
                         const req = await fetch("http://localhost:9000/users/cadastro",{
                             ...
                             req.status == 500 ? swal({title:"Atenção!", text:"Usuário já cadastrado tente novamente com um email diferente!", icon:"warning"}) : swal({title:"Parabens", text:"Voce sera redirecionado para pagina de login!", icon:"success", buttons:false}).then(setTimeout(function(){window.location.href="index.jsp";},5000));
                         }
                        }else{
